...
std::vector<uint8_t> knownJ1850Table = {
...
TEST(CrcTest, J1850TableGeneration) {
	Crc crc;
	std::vector<uint8_t> table;
	crc.gen_j1850_table(table);
	EXPECT_FALSE(table.empty());
	EXPECT_EQ(table.size(), 256); // Assuming the table should have 256 entries
	EXPECT_EQ(knownJ1850Table, table);
}

TEST(CrcTest, J1850TableRetrieval) {
	Crc crc;
	std::vector<uint8_t> table;
	crc.get_j1850_table(table);
	EXPECT_FALSE(table.empty());
	EXPECT_EQ(table.size(), 256); // Assuming the table should have 256 entries
	EXPECT_EQ(knownJ1850Table, table);
}

TEST(CrcTest, J1850Calculation) {
	Crc crc;
	...
	uint8_t startValue = 0xFF; // Example start value
	bool isFirstCall = false; // First call to the function

	uint8_t calculatedCRC = crc.calc_j1850(data, startValue, isFirstCall);
	EXPECT_EQ(calculatedCRC, 0x33);
}
